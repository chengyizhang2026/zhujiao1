...
import { ArrowLeft, Play, Loader2, Lightbulb, AlertTriangle, BookOpen, ChevronRight } from "lucide-react"
...
export default function GrammarPage() {
  ...
  async function loadContent(grammarTopic: string) {
    ...
    try {
      ...
      if (json.success) {
        setContent(json.data as Record<string, unknown>)
      } else {
    ...
    return (
      <div className="p-4 space-y-4 pb-24">
        <div className="pt-4 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-900">{grammar}</h1>
            <p className="text-xs text-gray-400">{mod?.title} · {topic}</p>
          </div>
        </div>
        ...
        {loading && (
          <div className="text-center py-16 space-y-4">
            ...
            <p className="text-sm text-gray-500">AI正在为你准备语法讲解...</p>
          </div>
        )}
        ...
        {error && (
          <div className="text-center py-16 space-y-4">
            ...
            <button onClick={() => loadContent(grammar)} className="px-4 py-2 bg-green-600 text-white text-sm rounded-lg">重试</button>
          </div>
        )}
        ...
        {content && !loading && (
          <>
            ...
            {(content.keyPoints as string[])?.length > 0 && (
              <div className="bg-amber-50 rounded-xl border border-amber-100 p-4">
                ...
                <ul className="space-y-1">
                  {(content.keyPoints as string[]).map((kp: string, i: number) => (
            ...
            {(content.commonMistakes as Array<{mistake: string; correction: string}>)?.length > 0 && (
              <div className="bg-red-50 rounded-xl border border-red-100 p-4">
                ...
                <div className="space-y-2">
                  {(content.commonMistakes as Array<{mistake: string; correction: string}>).map((cm, i: number) => (
            ...
            {(content.memoryTip as string) && (
              <div className="bg-green-50 rounded-xl border border-green-100 p-4">
                <h2 className="text-sm font-semibold text-green-700 mb-2">💡 记忆口诀</h2>
                <p className="text-green-900 font-medium">{content.memoryTip as string}</p>
              </div>
            )}
            ...
            {(content.examples as Array<{question: string; solution: string}>)?.length > 0 && (
              <div className="bg-white rounded-xl border border-gray-100 p-4">
                ...
                <div className="space-y-4">
                  {(content.examples as Array<{question: string; solution: string}>).map((ex, i: number) => (
                    <div key={i} className="p-3 bg-gray-50 rounded-lg">
                      <p className="text-sm font-medium text-gray-800 mb-2">{i + 1}. {ex.question}</p>
            ...
            <div className="flex gap-2">
              <Link
                href={`/english/vocab`}
                className="flex-1 py-3 bg-purple-50 text-purple-700 rounded-xl text-sm font-medium text-center hover:bg-purple-100 transition-colors"
              >
                📖 背单词
              </Link>
              <Link
                href={`/english/reading`}
                className="flex-1 py-3 bg-green-50 text-green-700 rounded-xl text-sm font-medium text-center hover:bg-green-100 transition-colors"
              >
                📚 读短文
              </Link>
            </div>
        ...
        {/* 返回单元列表 */}
        <button onClick={() => router.push("/english")} className="w-full py-3 bg-gray-100 text-gray-700 rounded-xl text-sm hover:bg-gray-200 transition-colors">
          返回英语首页
        </button>
      </div>
    )
  ...
  return (
    <div className="p-4 space-y-4 pb-20">
      <div className="pt-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-gray-900">📝 语法学习</h1>
          <p className="text-xs text-gray-400">外研版 · 七年级上册（2024新版）</p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {ENGLISH_MODULES.map((mod) => (
          <button
            key={mod.id}
            onClick={() => loadContent(mod.grammar[0])}
